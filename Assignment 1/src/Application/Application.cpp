...
//Texture3D::Sptr lut = ResourceManager::CreateAsset<Texture3D>("luts/coolshade.CUBE");//cool lut DONE
//Texture3D::Sptr lut2 = ResourceManager::CreateAsset<Texture3D>("luts/warm.CUBE");//warm lut NO 
Gameplay::Material::Sptr planeToonMaterial2;
...
void Application::_Update() {
	...
	if (app.CurrentScene()->FindObjectByName("Space Ship")->GetPosition().x > 60.f) {
		direction = direction * - 1;
		app.CurrentScene()->FindObjectByName("Space Ship")->SetPostion(glm::vec3(-30.f, app.CurrentScene()->FindObjectByName("Space Ship")->GetPosition().y, app.CurrentScene()->FindObjectByName("Space Ship")->GetPosition().z));
		app.CurrentScene()->FindObjectByName("Space Ship")->SetRotation(glm::vec3(20.f * direction, app.CurrentScene()->FindObjectByName("Space Ship")->GetRotation().y, 180.f));
	}
	app.CurrentScene()->FindObjectByName("Space Ship")->SetPostion(glm::vec3(app.CurrentScene()->FindObjectByName("Space Ship")->GetPosition().x + (0.1f), app.CurrentScene()->FindObjectByName("Space Ship")->GetPosition().y, app.CurrentScene()->FindObjectByName("Space Ship")->GetPosition().z));

	if ((InputEngine::GetKeyState(GLFW_KEY_9) == ButtonState::Pressed)) {
		app.CurrentScene()->SetColorLUT(ResourceManager::CreateAsset<Texture3D>("luts/coolshade.CUBE")); 
	}
	else if ((InputEngine::GetKeyState(GLFW_KEY_8) == ButtonState::Pressed)) {
		app.CurrentScene()->SetColorLUT(ResourceManager::CreateAsset<Texture3D>("luts/hott2.CUBE")); 
	}
